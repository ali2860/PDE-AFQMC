...
[-0.1044906, -1.0511902, -0.4899371, 0.0882198]]) # THIS IS T_MATRIX2 (4-sites)

# h1 = jnp.array([[ 3.52810469,  2.64105041,  1.9288264 ],
    #    [ 2.64105041,  3.73511598, -1.12863509],
    #    [ 1.9288264 , -1.12863509, -0.2064377 ]]) # THIS IS T_MATRIX1

# h1 = jnp.array([[-0.6680644, 0.9605467, 0.520678],

# [ 0.9605467, -0.1900202, 0.3938467],

# [ 0.520678 , 0.3938467 , 0.0340779]]) # THIS IS T_MATRIX2 (3-sites)
...
if n_elec[0] != 0 and n_elec[1] == 0:
    noise = np.random.uniform(-0.5, 0.5, hf_u.shape) # For n_sites = 4, n_elec = (2,0) t_matrix2
    # noise_up = np.random.uniform(-3, 3, hf_u.shape) # For n_sites = 3, n_elec = (1,0) t_matrix2
    hf_u += noise
    hf = [hf_u, hf_d]
elif n_elec[0] != 0 and n_elec[1] == 0:
    noise = np.random.uniform(-0.5, 0.5, hf_d.shape) # For n_sites = 4, n_elec = (2,0) t_matrix2
    # noise_up = np.random.uniform(-3, 3, hf_u.shape) # For n_sites = 3, n_elec = (1,0) t_matrix2
    hf_d += noise
hf = [hf_u, hf_d]
